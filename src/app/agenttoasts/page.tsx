...
const headerToneMap: Record<AgentSystemToastVariant, HeaderTone> = {
  runComplete: { dark: "#006A17", light: "#C7EBCF", title: "#005D15" },
  overrideRecommendation: { dark: "#A67700", light: "#EFE5BE", title: "#996800" },
  agentPaused: { dark: "#0B3D82", light: "#BCD3EE", title: "#0B3E86" },
  deleteAgent: { dark: "#ED1000", light: "#F3D7DA", title: "#6B1010" },
};
...
function CloseButton() {
  return (
    ...
    >
      <svg aria-hidden="true" className="size-5 sm:size-7 md:size-10" viewBox="0 0 24 24">
        <path d="M4 4L20 20M20 4L4 20" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.9" />
      </svg>
    </button>
  );
}
...
function HeaderIcon({ variant }: { variant: AgentSystemToastVariant }) {
  if (variant === "runComplete") {
    return (
      <svg aria-hidden="true" className="size-[clamp(16px,2.8vw,40px)]" viewBox="0 0 24 24">
        <path d="M3.5 12.5L9 18L20.5 6.5" fill="none" stroke="#ffffff" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.2" />
      </svg>
    );
  }
  if (variant === "overrideRecommendation") {
    return (
      <svg aria-hidden="true" className="size-[clamp(16px,2.8vw,40px)]" viewBox="0 0 24 24">
        <path d="M12 2.5L21.5 12L12 21.5L2.5 12L12 2.5Z" fill="none" stroke="#ffffff" strokeWidth="1.9" />
...
function HeaderBadge({ variant, title, width }: { variant: AgentSystemToastVariant; title: string; width: number }) {
  const tone = headerToneMap[variant];
  return (
    <div className="inline-flex h-[clamp(40px,7.2vw,94px)] max-w-full overflow-hidden" style={{ width: \`min(100%, \${width}px)\` }}>
      <div className="flex w-[clamp(46px,8.2vw,102px)] shrink-0 items-center justify-center" style={{ backgroundColor: tone.dark }}>
        <HeaderIcon variant={variant} />
      </div>
      <div className="flex min-w-0 flex-1 items-center px-[clamp(10px,2.4vw,34px)]" style={{ backgroundColor: tone.light }}>
        <p className="font-pixel truncate text-[clamp(14px,2.3vw,30px)] leading-none tracking-[0.045em]" style={{ color: tone.title }}>
          {title}
        </p>
      </div>
    </div>
...
function ActionButton({ children, variant = "ghost" }: { children: React.ReactNode; variant?: "ghost" | "dark" | "danger" }) {
  return (
    <button
      type="button"
      className={cn(
...
function RunCompleteToast() {
  return (
    <ToastShell>
      ...
      <MainBody>
        <p>The agent finished processing 847 records<br className="hidden sm:block" /> from &quot;Q1 Sale Data.csv.&quot;</p>
        <p><strong>3 rows were skipped,</strong> mismatched column<br className="hidden sm:block" /> headers.</p>
...
function OverrideRecommendationToast() {
  return (
    <ToastShell>
      ...
      <MainBody>
        <p>The agent recommended pausing the<br className="hidden sm:block" /> *Ad Campaign* due to low CTR (0.3%).</p>
        <p>You&apos;re about to resume it.<br className="hidden sm:block" /> The agent will continue spending ~\$140/day.</p>
      </MainBody>
...
function AgentPausedToast() {
  return (
    <ToastShell>
      ...
      <MainBody>
        <p>The *Email Responder* agent paused after<br className="hidden sm:block" /> detecting an unusual reply pattern.</p>
        <p>It sent <strong>14 responses before stopping.</strong><br className="hidden sm:block" /> No further emails will be sent until you<br className="hidden sm:block" /> resume.</p>
      </MainBody>
...
function DeleteAgentToast() {
  return (
    <ToastShell>
      ...
      <MainBody>
        <p>Deleting *Weekly Digest* will permanently<br className="hidden sm:block" /> delete everything. This cannot be undone.</p>
        <p>Type <strong>&quot;Weekly Digest&quot;</strong> to confirm:</p>
      </MainBody>
      <div className="mt-6 sm:mt-7">
        <input type="text" value="Weekly Digest" readOnly aria-label="Agent name confirmation" className="h-[clamp(42px,6.2vw,58px)] w-full rounded-[clamp(12px,2vw,20px)] border border-[#cfd0d4] bg-[#f3f3f4] px-4 text-[clamp(14px,2.1vw,26px)] leading-none text-[#1e1f23] outline-none" />
      </div>
      <div className="mt-6 sm:mt-7">
...
function AgentSystemToast({ variant, className }: { variant: AgentSystemToastVariant; className?: string }) {
  return (
...
export default function AgentToastsPage() {
  const [activeVariant, setActiveVariant] = useState<AgentSystemToastVariant>("runComplete");
