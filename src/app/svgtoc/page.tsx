...
// Core idea: build an SVG path with cubic Bezier curves connecting
// TOC items at varying indent depths, then animate a highlight using
// clip-path (not mask-image) to avoid layout recalculation.

// 1. Build outline path
function buildOutlinePath(items) {
  let d = "";
  let runningY = 0;
  for (let i = 0; i < items.length; i++) {
    const offsetX = STROKE_X_BASE + items[i].depth * INDENT_PX;
    const topY = runningY;
    const bottomY = topY + ITEM_H;
    if (i === 0) {
      d += \`M \${offsetX} \${topY} L \${offsetX} \${bottomY}\`;
    } else {
      const upperOffsetX = STROKE_X_BASE + items[i - 1].depth * INDENT_PX;
      d += \` C \${upperOffsetX} \${topY - 4} \${offsetX} \${topY - GAP_H + 4} \${offsetX} \${topY}\`;
      d += \` L \${offsetX} \${bottomY}\`;
    }
    runningY = bottomY + GAP_H;
  }
...
// 2. Animate with clip-path (not mask-image + transform)
<div style={{
  clipPath: \`inset(\${thumbTop}px 0 \${totalH - thumbBottom}px 0)\`,
  transition: "clip-path 250ms ease",
}}>
  <svg><path d={outlinePath} stroke="#8162ff" /></svg>
</div>

// 3. Thumb box
<div style={{
  translate: \`\${offsetX - 3}px \${thumbTop - 3}px\`,
