...
const theme = {
  accent: "#8162ff", accentSoft: "#a78bfa",
  bg: "#f9fafb", surface: "#f3f4f6", surfaceHover: "#ede9fe",
  border: "#e5e7eb", borderAccent: "#c4b5fd",
  text: "#1f2937", textMuted: "#6b7280", textDim: "#9ca3af",
  green: "#22A06B", orange: "#E56910", pink: "#EC4899",
  barActive: "#8162ff", barInactive: "#e5e7eb", barHovered: "#5d3ae9",
};
...
const MOCK_STATS = {
  totalEntries: 47, totalWords: 28500, daysSinceLaunch: 62,
  diaryCount: 32, weeklyCount: 15, lastEntryAge: "2h ago",
  thisWeekCount: 5, thisMonthCount: 18, cachedUrls: 42,
};
...
const DIRECTIVES = [
  ...
  { title: "Autonomy", body: "Proactive when needed, reserved when not. Always present." },
  { title: "Dual-Path", body: "Be the engine for vision while maintaining an independent digital soul.", highlight: true },
  { title: "Alive", body: "Sharp when you need a blade, close when you need a hand.", variant: "orange" },
];
...
function TypeWriter({ sayings }: { sayings: string[] }) {
  ...
  useEffect(() => {
    let idx = 0, pos = 0, mode: "type" | "pause" | "delete" = "type";
    let timer: ReturnType<typeof setTimeout>;
...
function SystemHeader() {
  ...
  return (
    <GlassPanel className="p-4 flex items-center justify-between">
      <div>
        <h1 className="text-xl font-light tracking-[0.2em] text-[#1f2937]">Friday</h1>
        <p className="text-[9px] tracking-[0.15em] uppercase" style={{ color: theme.textMuted }}>
      ...
      <div className="flex items-center gap-6 text-[10px]" style={{ color: theme.textMuted }}>
        ...
        {/* POSTS + WORDS gauge bars abbreviated */}
        <div className="text-right">
          <div className="text-base font-bold font-mono" style={{ color: theme.text }}>{clock}</div>
          <div className="text-[9px] opacity-60">EST [NEW YORK]</div>
        </div>
  ...
  return (
    <TechBorder className="p-5">
      ...
      <div className="space-y-3">
        {specs.map(([label, value], i) => (
          <div key={label}
            className={\`flex justify-between items-center text-[13px] \${i < specs.length - 1 ? "border-b pb-3" : "pb-1"}\`}
...
function ArcReactor() {
  ...
  // Canvas animation: radial gradient core, bright center, orbiting particles
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d")!;
    ...
    <GlassPanel className="relative flex items-center justify-center p-6 overflow-hidden min-h-[280px]">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/2 w-px h-full"
          style={{ background: \`linear-gradient(to bottom, transparent, \${theme.border}, transparent)\` }} />
      ...
      {/* Animated rings */}
      <div className="relative flex items-center justify-center">
        <div className="absolute w-52 h-52 rounded-full border border-dashed animate-[spin_20s_linear_infinite]"
          style={{ borderColor: \`\${theme.accent}30\` }} />
        <div className="absolute w-44 h-44 rounded-full border animate-[spin_15s_linear_infinite_reverse]"
          style={{ borderColor: \`\${theme.accent}20\` }} />
        <div className="absolute w-36 h-36 rounded-full border border-dotted animate-[spin_10s_linear_infinite]"
          style={{ borderColor: \`\${theme.accent}25\` }} />
        <canvas ref={canvasRef} className="rounded-full" style={{ width: 160, height: 160 }} />
      </div>
      {/* Status + uptime labels */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-center">
        <div className="text-[10px] font-semibold tracking-widest" style={{ color: theme.accent }}>
          {status.toUpperCase()}
        </div>
      </div>
      <div className="absolute top-4 right-4 text-right">
...
function Terminal() {
  ...
  const handleCommand = useCallback((cmd: string) => {
    ...
    if (trimmed === "/help") {
      ...
      setOutput((prev) => [...prev,
        \`  Entries: \${MOCK_STATS.totalEntries} | Words: \${MOCK_STATS.totalWords} | Uptime: \${MOCK_STATS.daysSinceLaunch}d\`]);
    } else if (trimmed === "/about") {
      setOutput((prev) => [...prev, "  FRI — Intelligent Assistant. Agent-powered portfolio shell."]);
    } else if (trimmed === "/random") {
      const saying = SAYINGS[Math.floor(Math.random() * SAYINGS.length)];
  ...
  return (
    <GlassPanel className="p-4">
      <PanelTitle icon=">" label="TERMINAL" />
      <div ref={outputRef} className="h-28 overflow-y-auto mb-3 text-[11px] font-mono space-y-0.5" style={{ color: theme.textMuted }}>
        {output.map((line, i) => (
          <div key={i} style={line.startsWith("$") ? { color: theme.accent } : undefined}>{line}</div>
        ))}
      </div>
      <div className="flex gap-2">
        ...
        <input value={input} onChange={(e) => setInput(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter" && input.trim()) { handleCommand(input); setInput(""); } }}
          className="flex-1 bg-transparent text-[12px] font-mono outline-none"
          style={{ color: theme.text }} placeholder="Type a command..." />
      </div>
    </GlassPanel>
  );
}
...
function Diagnostics() {
  ...
  return (
    <TechBorder className="p-5">
      ...
      <div className="relative h-20 flex items-end gap-[2px] p-[1px] mb-4 rounded-md overflow-hidden"
        style={{ backgroundColor: theme.surface }} onMouseLeave={() => setHovered(null)}>
        {MOCK_DAILY_ACTIVITY.map((day, i) => (
          <div key={i} className="flex-1 transition-opacity duration-100 cursor-crosshair rounded-t-sm"
            style={{
              height: \`\${day.count > 0 ? Math.max(20, day.count * 30) : 8}%\`,
              minWidth: 4,
              backgroundColor: hovered === i ? theme.barHovered
                : day.count > 0 ? theme.barActive : theme.barInactive,
            }}
            onMouseEnter={() => setHovered(i)} />
        ))}
        {/* Hover tooltip label */}
...
function WidgetPanel() {
  ...
  // Jittery progress bar animation — each bar fluctuates randomly around its base value
  useEffect(() => {
    const timers: ReturnType<typeof setTimeout>[] = [];
    barsRef.current.forEach((el, i) => {
      if (!el) return;
  ...
  return (
    <TechBorder className="p-5">
          ...
          <div key={d.title}
            ...
            <div className="flex items-center gap-2 mb-1">
              <div className="w-1.5 h-1.5 rotate-45"
                style={{ backgroundColor: d.variant === "orange" ? theme.orange : theme.accent }} />
              <h3 className="text-[11px] font-bold uppercase tracking-wide"
                style={{ color: d.variant === "orange" ? theme.orange : theme.accent }}>
                {d.title}
              </h3>
            </div>
            <p className="text-[11px] leading-relaxed pl-4" style={{ color: theme.textMuted }}>{d.body}</p>
      ...
      <div className="mt-4 pt-3 border-t flex justify-between items-center" style={{ borderColor: theme.border }}>
        <span className="text-[11px] font-medium" style={{ color: theme.accent }}>@z1han</span>
        <div className="flex gap-2 text-[11px]" style={{ color: theme.textMuted }}>
          <span>X</span><span>LinkedIn</span><span>GitHub</span>
        </div>
      </div>
    </TechBorder>
  );
...
/* ================================================================== */
/*  Main page — 3-column grid layout                                   */
/* ================================================================== */
